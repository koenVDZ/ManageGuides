...
* @subpackage	Classes
*/
class GuidemanCkModelPhonesitem extends GuidemanClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'phonesitemown', 'STRING');
	}
	...
	*/
	public function getTable($type = 'phonesitem', $prefix = 'GuidemanTable', $config = array())
	{
	...
	*/
	...
	{
		return parent::hit(array('phonesitem'));
	}
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_guideman.edit.phonesitem.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('phonesitem.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
				...
				$data->default_phone = 0;
				$data->label = $jinput->get('filter_label', $this->getState('filter.label'), 'INT');
				$data->cdc = $jinput->get('filter_cdc', $this->getState('filter.cdc'), 'INT');
				$data->number = null;
				$data->operator = $jinput->get('filter_operator', $this->getState('filter.operator'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the phonesitem
	*
	* @return	void
	*/
		...
		switch($this->getState('context', 'all'))
		{
			...
			case 'layout.phonesitemown':
				...
				$this->orm->select(array(
					...
					'operator',
					'operator.country_id',
					'operator.country_id.country_name',
					'operator.operator',
					'user_id',
					'user_id.alias'
				));
...
if (!class_exists('GuidemanModelPhonesitem')){ class GuidemanModelPhonesitem extends GuidemanCkModelPhonesitem{} }
