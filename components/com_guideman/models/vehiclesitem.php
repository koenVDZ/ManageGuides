...
* @subpackage	Classes
*/
class GuidemanCkModelVehiclesitem extends GuidemanClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'vehicleitem', 'STRING');
	}
	...
	*/
	public function getTable($type = 'vehiclesitem', $prefix = 'GuidemanTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_guideman.edit.vehiclesitem.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('vehiclesitem.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
				...
				$data->vehicle_type = $jinput->get('filter_vehicle_type', $this->getState('filter.vehicle_type', '1'), 'STRING');
				$data->brand_id = $jinput->get('filter_brand_id', $this->getState('filter.brand_id'), 'INT');
				$data->model = null;
				$data->color = null;
				$data->pax = null;
				$data->number_plate = null;
				$data->year_of_build = null;
				$data->fuel = $jinput->get('filter_fuel', $this->getState('filter.fuel', ''), 'STRING');
				$data->car_insurance = null;
				$data->insurance_for_third_parties = null;
				$data->remarks = null;
	...
	* @param	integer	$pk	The primary id key of the vehiclesitem
	*
	* @return	void
	*/
	...
	*/
	...
	{
		//Convert from a non-SQL formated date (year_of_build)
		$data['year_of_build'] = GuidemanHelperDates::getSqlDate($data['year_of_build'], array('d-m-Y'), true, 'USER_UTC');
...
if (!class_exists('GuidemanModelVehiclesitem')){ class GuidemanModelVehiclesitem extends GuidemanCkModelVehiclesitem{} }
