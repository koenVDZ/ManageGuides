...
* @subpackage	Classes
*/
class GuidemanCkModelJobsitem extends GuidemanClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'jobsedit', 'STRING');
	}
	...
	*/
	public function getTable($type = 'jobsitem', $prefix = 'GuidemanTable', $config = array())
	{
	...
	*/
	...
	{
		return parent::hit(array('jobsitemfly'));
	}
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_guideman.edit.jobsitem.data', array());
		...
		switch($this->getState('context', 'all'))
		{
			case 'layout.jobsitemfly':
				...
				$this->orm->select(array(
					...
					'trip_leader.alias',
					'trip_leader.name',
					'trip_leader.nationality',
					'trip_leader.nationality.iso_2',
					'trip_leader.surname'
				));

				$this->orm->relation('jobitems', array(
					'select' => 'job_id,type,item_status,start_date,start_time,end_date,end_time,remark,service_provider_status,guide_status,transport_status,pax'
				));

				$this->orm->relation('services', array(
					'select' => 'company,service_name'
				));

				$this->orm->relation('contacts', array(
					'select' => 'name,user_id,name,surname,alias,name,user_id,name,surname,name,user_id,name,surname,name,user_id,name,surname'
				));
...
if (!class_exists('GuidemanModelJobsitem')){ class GuidemanModelJobsitem extends GuidemanCkModelJobsitem{} }
