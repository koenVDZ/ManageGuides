...
* @subpackage	Contact
*/
class GuidemanCkViewContact extends GuidemanClassView
{
	/**
	...
	* @var array
	*/
	protected $layouts = array('contact', 'company', 'contactall', 'contactinfo');
	...
	*/
	...
	/**
	* Execute and display a template : Contact Information
	*
	* @access	protected
	* @param	string	$tpl	The name of the template file to parse; automatically searches through the template paths.
	*
	...
	* @return	mixed	A string if successful, otherwise a JError object.
	*/
	protected function displayContactall($tpl = null)
	{
		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$this->params 	= $state->get('params');
		$state->set('context', 'layout.contactall');
		$this->item		= $item		= $this->get('Item');
		$this->canDo	= $canDo	= GuidemanHelper::getActions($model->getId());
		$lists = array();
		$this->lists = &$lists;
		...
		// Define the title
		$this->_prepareDocument(JText::_('GUIDEMAN_LAYOUT_CONTACT_INFORMATION'), $this->item, 'name');
	...
	/**
	* Execute and display a template : Company Information
	*
	* @access	protected
	* @param	string	$tpl	The name of the template file to parse; automatically searches through the template paths.
	*
	...
	* @return	mixed	A string if successful, otherwise a JError object.
	*/
	protected function displayContactinfo($tpl = null)
	{
		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$this->params 	= $state->get('params');
		$state->set('context', 'layout.contactinfo');
		$this->item		= $item		= $this->get('Item');
		$this->canDo	= $canDo	= GuidemanHelper::getActions($model->getId());
		$lists = array();
		$this->lists = &$lists;
		...
		// Define the title
		$this->_prepareDocument(JText::_('GUIDEMAN_LAYOUT_COMPANY_INFORMATION'), $this->item, 'name');
...
if (!class_exists('GuidemanViewContact')){ class GuidemanViewContact extends GuidemanCkViewContact{} }
