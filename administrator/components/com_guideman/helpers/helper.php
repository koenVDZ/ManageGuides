...
/**
* Guideman Helper functions.
*
* @package	Guideman
* @subpackage	Helper
*/
class GuidemanCkHelper
{
	/**
	...
	*/
	...
	{
		return GuidemanHelperMenu::addSubmenu($alias, $view, $layout);
	}
	...
	*/
	public static function enumList($ctrl, $fieldName)
	{
		...
		return GuidemanHelperEnum::_($ctrl . '_' . $fieldName);
	}
	...
	*/
	public static function getActions($itemId = 0)
	{
		...
		foreach ($actions as $action)
			$result->set($action, $user->authorise($action, COM_GUIDEMAN));
	...
	*/
	...
	{
		if (is_array($attribs))
			$attribs = GuidemanHelperFile::getAttributesFromLegacy($attribs);

		return GuidemanHelperFile::getFileUrl($path, $method, $attribs);
	}
	...
	*/
	public static function headerDeclarations()
	{
		...
		$baseSite = 'components/' . COM_GUIDEMAN;
		$baseAdmin = 'administrator/components/' . COM_GUIDEMAN;
		...
		GuidemanHelperHtmlValidator::loadLanguageScript();
		GuidemanHelperHtmlValidator::attachForm();
		...
		if ($app->isAdmin())
		{
			...
			self::addStyleSheet($baseAdmin, 'css/guideman.css');
			self::addStyleSheet($baseAdmin, 'css/toolbar.css');

		}
		else if ($app->isSite())
		{
			self::addStyleSheet($baseSite, 'css/guideman.css');
	...
	*/
	public static function loadFork($file)
	{
		...
		$file = preg_replace("#com_guideman#", 'com_guideman/fork', $file);
...
// Load the fork
GuidemanCkHelper::loadFork(__FILE__);

// Fallback if no fork has been found
if (!class_exists('GuidemanHelper')){ class GuidemanHelper extends GuidemanCkHelper{} }
