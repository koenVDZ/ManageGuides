...
class GuidemanCkHelperForm
{
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = GuidemanHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'guideman.') . $relation->model;

				$model = GuidemanHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = GuidemanClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return GuidemanHelper::componentModel($model, $item);
	}
...
if (!class_exists('GuidemanHelperForm')){ class GuidemanHelperForm extends GuidemanCkHelperForm{} }
