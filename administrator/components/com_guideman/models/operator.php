...
* @subpackage	Classes
*/
class GuidemanCkModelOperator extends GuidemanClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	...
	{
		$conditions = array('`country_id` = '.(int) $table->country_id);
		return $conditions;
	}
	...
	*/
	public function getTable($type = 'operator', $prefix = 'GuidemanTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_guideman.edit.operator.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('operator.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
				...
				$data->operator = null;
				$data->contact_id = $jinput->get('filter_contact_id', $this->getState('filter.contact_id'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the operator
	*
	* @return	void
	*/
...
if (!class_exists('GuidemanModelOperator')){ class GuidemanModelOperator extends GuidemanCkModelOperator{} }
