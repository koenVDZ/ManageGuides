...
* @subpackage	Classes
*/
class GuidemanCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = GuidemanClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("GUIDEMAN_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_guideman' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!GuidemanHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			case 'catid': return $tbl.$com. 'categories';
			case 'user_id': return $tbl.$com. 'contacts';
			case 'bank_id': return $tbl.$com. 'contacts';
			case 'created_by': return $tbl. 'users';
			case 'modified_by': return $tbl. 'users';
			case 'language': return $tbl.$com. 'lang';
			case 'address_label': return $tbl.$com. 'addresslabels';
			case 'country_id': return $tbl.$com. 'countries';
			case 'state_id': return $tbl.$com. 'states';
			case 'company_id': return $tbl.$com. 'contacts';
			case 'business_type': return $tbl.$com. 'businesstypes';
			case 'nationality': return $tbl.$com. 'countries';
			case 'visits_id': return $tbl.$com. 'categories';
			case 'my_joomla_access': return $tbl. 'viewlevels';
			case 'my_joomla_user': return $tbl. 'users';
			case 'label_id': return $tbl.$com. 'doclabels';
			case 'client_id': return $tbl.$com. 'contacts';
			case 'requested_language': return $tbl.$com. 'languages';
			case 'second_language_option': return $tbl.$com. 'languages';
			case 'operator_name': return $tbl.$com. 'contacts';
			case 'main_guide': return $tbl.$com. 'contacts';
			case 'trip_leader': return $tbl.$com. 'contacts';
			case 'remunerations': return $tbl.$com. 'prices';
			case 'invoicing': return $tbl.$com. 'prices';
			case 'job_id': return $tbl.$com. 'jobs';
			case 'service': return $tbl.$com. 'services';
			case 'service_provider': return $tbl.$com. 'contacts';
			case 'guide': return $tbl.$com. 'contacts';
			case 'transport': return $tbl.$com. 'contacts';
			case 'driver': return $tbl.$com. 'contacts';
			case 'access': return $tbl. 'viewlevels';
			case 'contact_id': return $tbl.$com. 'contacts';
			case 'label': return $tbl.$com. 'phonelabels';
			case 'cdc': return $tbl.$com. 'countries';
			case 'operator': return $tbl.$com. 'operators';
			case 'company': return $tbl.$com. 'contacts';
			case 'currency': return $tbl.$com. 'currencies';
			case 'country': return $tbl.$com. 'countries';
			case 'state': return $tbl.$com. 'states';
			case 'remunaration': return $tbl.$com. 'prices';
			case 'costs': return $tbl.$com. 'prices';
			case 'policy': return $tbl.$com. 'policies';
			case 'labelid': return $tbl.$com. 'sociallabels';
			case 'brand_id': return $tbl.$com. 'brands';	
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'GuidemanModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_GUIDEMAN . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'GuidemanModel');
		if ($modelList)
	...
	*/
	public function integrityRestrict($relation, &$pks, $labelKey, $parentLabelKey, $editLink)
	{
		...
		foreach($pks as $id)
		{
			...
			$children = GuidemanHelper::getData($childModel, array(
				'filter' => array(
			...
			if (count($children))
			{
				...
				$model = GuidemanHelper::componentModel($relation->foreignModelClass, true);
				...
				$msg = sprintf(JText::_('GUIDEMAN_ERROR_IMPOSSIBLE_TO_DELETE_USED_IN'), ($itemName?$itemName:'item'), implode(', ', $childText));
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_GUIDEMAN . '/models/forms');
		JForm::addFieldPath(JPATH_GUIDEMAN . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = GuidemanClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'guidemanModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Guideman"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_GUIDEMAN . '/fork/models/forms';
	...
	*/
	...
	{
		return GuidemanClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("GUIDEMAN_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !GuidemanHelperDates::isNull((string)$value))
						{
							$time = GuidemanHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('GUIDEMAN_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
...
if (!class_exists('GuidemanClassModelItem')){ class GuidemanClassModelItem extends GuidemanCkClassModelItem{} }
