...
* @subpackage	Classes
*/
class GuidemanCkClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'GuidemanModel');
	...
	*/
	public function getItems()
	{
		try
		{
			...
			{
				if (!$this->canAdmin())
					$error = JText::_('GUIDEMAN_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'GuidemanModel');
	...
	*/
	...
	{
		$url = 'index.php?option=com_guideman';
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_guideman', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = GuidemanHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'GuidemanModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__guideman_' . $this->getName())
...
if (!class_exists('GuidemanClassModelList')){ class GuidemanClassModelList extends GuidemanCkClassModelList{} }
