...
* @subpackage	
*/
class GuidemanCkClassControllerItem extends JControllerForm
{
	/**
	...
	* @var string
	*/
	protected $text_prefix = 'COM_GUIDEMAN';
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_guideman.' . $this->view_list . '.default',
			'com_guideman.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	protected function applyRedirection($result, $redirections, $vars = array())
	{
		...
		switch($redirection)
		{
			...
			case 'stay':
				$this->setRedirect(GuidemanHelper::urlRequest($vars));
				return;
				break;
		...
		//Filters followers
		$model = CkJModel::getInstance($this->view_list, 'GuidemanModel');
		if ($model)
		{
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('GUIDEMAN_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	public function redirect()
	{
		if ($this->redirect)
		{
			...
			if ($jinput->get('return') == 'json')
			{
				...
				GuidemanClassAjax::responseJson(array(
	...
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		...
		try
		{
			$changedValue = GuidemanHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
...
if (!class_exists('GuidemanClassControllerItem')){ class GuidemanClassControllerItem extends GuidemanCkClassControllerItem{} }
