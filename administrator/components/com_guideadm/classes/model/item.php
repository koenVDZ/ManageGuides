...
* @subpackage	Classes
*/
class GuideadmCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = GuideadmClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("GUIDEADM_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_guideadm' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!GuideadmHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'GuideadmModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_GUIDEADM . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'GuideadmModel');
		if ($modelList)
	...
	*/
	public function integrityRestrict($relation, &$pks, $labelKey, $parentLabelKey, $editLink)
	{
		...
		foreach($pks as $id)
		{
			...
			$children = GuideadmHelper::getData($childModel, array(
				'filter' => array(
			...
			if (count($children))
			{
				...
				$model = GuideadmHelper::componentModel($relation->foreignModelClass, true);
				...
				$msg = sprintf(JText::_('GUIDEADM_ERROR_IMPOSSIBLE_TO_DELETE_USED_IN'), ($itemName?$itemName:'item'), implode(', ', $childText));
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_GUIDEADM . '/models/forms');
		JForm::addFieldPath(JPATH_GUIDEADM . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = GuideadmClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'guideadmModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Guideadm"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_GUIDEADM . '/fork/models/forms';
	...
	*/
	...
	{
		return GuideadmClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("GUIDEADM_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !GuideadmHelperDates::isNull((string)$value))
						{
							$time = GuideadmHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('GUIDEADM_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
...
if (!class_exists('GuideadmClassModelItem')){ class GuideadmClassModelItem extends GuideadmCkClassModelItem{} }
