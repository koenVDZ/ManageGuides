...
* @subpackage	Classes
*/
class GuideadmCkClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'GuideadmModel');
	...
	*/
	public function getItems()
	{
		try
		{
			...
			{
				if (!$this->canAdmin())
					$error = JText::_('GUIDEADM_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'GuideadmModel');
	...
	*/
	...
	{
		$url = 'index.php?option=com_guideadm';
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_guideadm', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = GuideadmHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'GuideadmModel');
...
if (!class_exists('GuideadmClassModelList')){ class GuideadmClassModelList extends GuideadmCkClassModelList{} }
