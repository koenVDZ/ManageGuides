...
*/
class GuideadmCkViewContactlangitem extends GuideadmClassView
{
	/**
	...
	*/
	protected function displayContactlangitem($tpl = null)
	{
		...
		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_CONTACTLANG_ITEM'), $this->item, 'user_id');
		...
		//Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_CONTACTLANG_ITEM'), 'pencil-2');

		// Save & Close
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::save('contactlangitem.save', "GUIDEADM_JTOOLBAR_SAVE_CLOSE");
		// Save
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::apply('contactlangitem.apply', "GUIDEADM_JTOOLBAR_SAVE");
		// Cancel
		JToolBarHelper::cancel('contactlangitem.cancel', "GUIDEADM_JTOOLBAR_CANCEL");
		// Publish
		if (!$isNew && $model->canEditState($item) && ($item->published != 1))
			JToolBarHelper::publish('contactlang.publish', "GUIDEADM_JTOOLBAR_PUBLISH");
		// Unpublish
		if (!$isNew && $model->canEditState($item) && ($item->published != 0))
			JToolBarHelper::unpublish('contactlang.unpublish', "GUIDEADM_JTOOLBAR_UNPUBLISH");
		// Trash
		if (!$isNew && $model->canEditState($item) && ($item->published != -2))
			JToolBarHelper::trash('contactlang.trash', "GUIDEADM_JTOOLBAR_TRASH", false);
		// Archive
		if (!$isNew && $model->canEditState($item) && ($item->published != 2))
			JToolBarHelper::custom('contactlang.archive', 'archive', 'archive',  "GUIDEADM_JTOOLBAR_ARCHIVE", false);
		...
		$model_language = CkJModel::getInstance('Languages', 'GuideadmModel');
		...
		//Ordering
		$orderModel = CkJModel::getInstance('Contactlang', 'GuideadmModel');
...
if (!class_exists('GuideadmViewContactlangitem')){ class GuideadmViewContactlangitem extends GuideadmCkViewContactlangitem{} }
