...
*/
class GuideadmCkViewDocumentsitem extends GuideadmClassView
{
	/**
	...
	*/
	protected function displayDocument($tpl = null)
	{
		...
		$state->set('context', 'layout.document');
		...
		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_DOCUMENT'), $this->item, 'user_id');
		...
		//Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_DOCUMENT'), 'pencil-2');

		// Save & Close
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::save('documentsitem.save', "GUIDEADM_JTOOLBAR_SAVE_CLOSE");
		// Save
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::apply('documentsitem.apply', "GUIDEADM_JTOOLBAR_SAVE");
		// Cancel
		JToolBarHelper::cancel('documentsitem.cancel', "GUIDEADM_JTOOLBAR_CANCEL");
		// Publish
		if (!$isNew && $model->canEditState($item) && ($item->published != 1))
			JToolBarHelper::publish('documents.publish', "GUIDEADM_JTOOLBAR_PUBLISH");
		// Unpublish
		if (!$isNew && $model->canEditState($item) && ($item->published != 0))
			JToolBarHelper::unpublish('documents.unpublish', "GUIDEADM_JTOOLBAR_UNPUBLISH");
		// Trash
		if (!$isNew && $model->canEditState($item) && ($item->published != -2))
			JToolBarHelper::trash('documents.trash', "GUIDEADM_JTOOLBAR_TRASH", false);
		// Archive
		if (!$isNew && $model->canEditState($item) && ($item->published != 2))
			JToolBarHelper::custom('documents.archive', 'archive', 'archive',  "GUIDEADM_JTOOLBAR_ARCHIVE", false);
		...
		$model_label_id = CkJModel::getInstance('Doclabels', 'GuideadmModel');
		$model_label_id->addGroupOrder("a.doc_type_name");
		$lists['fk']['label_id'] = $model_label_id->getItems();
		...
		//Ordering
		$orderModel = CkJModel::getInstance('Documents', 'GuideadmModel');
...
if (!class_exists('GuideadmViewDocumentsitem')){ class GuideadmViewDocumentsitem extends GuideadmCkViewDocumentsitem{} }
