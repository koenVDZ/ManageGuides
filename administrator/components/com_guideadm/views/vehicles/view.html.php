...
* @subpackage	Vehicles
*/
class GuideadmCkViewVehicles extends GuideadmClassView
{
	/**
	...
	*/
	protected function displayDefault($tpl = null)
	{
		...
		$this->menu = GuideadmHelper::addSubmenu('vehicles', 'default');
		$lists = array();
		$this->lists = &$lists;

		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_VEHICLES'));
		...
		// Brand ID > Name
		$modelBrand_id = CkJModel::getInstance('brands', 'GuideadmModel');
		...
		// Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_VEHICLES'), 'list');

		// New
		if ($model->canCreate())
			JToolBarHelper::addNew('vehiclesitem.add', "GUIDEADM_JTOOLBAR_NEW");

		// Edit
		if ($model->canEdit())
			JToolBarHelper::editList('vehiclesitem.edit', "GUIDEADM_JTOOLBAR_EDIT");

		// Delete
		if ($model->canDelete())
			JToolBarHelper::deleteList(JText::_('GUIDEADM_JTOOLBAR_ARE_YOU_SURE_TO_DELETE'), 'vehiclesitem.delete', "GUIDEADM_JTOOLBAR_DELETE");
		...
		if ($model->canEditState())
			JToolBarHelper::publishList('vehicles.publish', "GUIDEADM_JTOOLBAR_PUBLISH");

		// Unpublish
		if ($model->canEditState())
			JToolBarHelper::unpublishList('vehicles.unpublish', "GUIDEADM_JTOOLBAR_UNPUBLISH");

		// Trash
		if ($model->canEditState())
			JToolBarHelper::trash('vehicles.trash', "GUIDEADM_JTOOLBAR_TRASH");

		// Archive
		if ($model->canEditState())
			JToolBarHelper::archiveList('vehicles.archive', "GUIDEADM_JTOOLBAR_ARCHIVE");
	}
	...
	*/
	protected function displayModal($tpl = null)
	{
		...
		$this->menu = GuideadmHelper::addSubmenu('vehicles', 'modal');
		$lists = array();
		$this->lists = &$lists;
...
if (!class_exists('GuideadmViewVehicles')){ class GuideadmViewVehicles extends GuideadmCkViewVehicles{} }
