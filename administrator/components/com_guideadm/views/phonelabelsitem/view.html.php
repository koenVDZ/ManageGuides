...
class GuideadmCkViewPhonelabelsitem extends GuideadmClassView
{
	...
	* @var array
	*/
	protected $layouts = array('phonelabel');

	/**
	* Execute and display a template : Phone Label
	*
	* @access	protected
	...
	*/
	protected function displayPhonelabel($tpl = null)
	{
		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$this->params 	= $state->get('params');
		$state->set('context', 'layout.phonelabel');
		...
		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_PHONE_LABEL'), $this->item, 'type');
		...
		//Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_PHONE_LABEL'), 'pencil-2');
		...
		// Save
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::apply('phonelabelsitem.apply', "GUIDEADM_JTOOLBAR_SAVE");
		// Cancel
		JToolBarHelper::cancel('phonelabelsitem.cancel', "GUIDEADM_JTOOLBAR_CANCEL");
		// Publish
		if (!$isNew && $model->canEditState($item) && ($item->published != 1))
			JToolBarHelper::publish('phonelabels.publish', "GUIDEADM_JTOOLBAR_PUBLISH");
		// Unpublish
		if (!$isNew && $model->canEditState($item) && ($item->published != 0))
			JToolBarHelper::unpublish('phonelabels.unpublish', "GUIDEADM_JTOOLBAR_UNPUBLISH");
		// Trash
		if (!$isNew && $model->canEditState($item) && ($item->published != -2))
			JToolBarHelper::trash('phonelabels.trash', "GUIDEADM_JTOOLBAR_TRASH", false);
		// Archive
		if (!$isNew && $model->canEditState($item) && ($item->published != 2))
			JToolBarHelper::custom('phonelabels.archive', 'archive', 'archive',  "GUIDEADM_JTOOLBAR_ARCHIVE", false);
		...
		//Ordering
		$orderModel = CkJModel::getInstance('Phonelabels', 'GuideadmModel');
