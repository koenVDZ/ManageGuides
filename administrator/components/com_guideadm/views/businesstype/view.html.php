...
class GuideadmCkViewBusinesstype extends GuideadmClassView
{
	...
	* @var array
	*/
	protected $layouts = array('businesstype');

	/**
	* Execute and display a template : BusinessType
	*
	* @access	protected
	...
	*/
	protected function displayBusinesstype($tpl = null)
	{
		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$this->params 	= $state->get('params');
		$state->set('context', 'layout.businesstype');
		...
		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_BUSINESSTYPE'), $this->item, 'country_id');
		...
		//Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_BUSINESSTYPE'), 'pencil-2');
		...
		//Ordering
		$orderModel = CkJModel::getInstance('Businesstypes', 'GuideadmModel');
