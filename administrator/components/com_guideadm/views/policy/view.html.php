...
class GuideadmCkViewPolicy extends GuideadmClassView
{
	...
	*/
	protected function displayPolicy($tpl = null)
	{
		...
		//Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_POLICY'), 'pencil-2');
		...
		// Save
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::apply('policy.apply', "GUIDEADM_JTOOLBAR_SAVE");
		// Cancel
		...
		//Ordering
		$orderModel = CkJModel::getInstance('Policies', 'GuideadmModel');
