...
* @subpackage	Social
*/
class GuideadmCkViewSocial extends GuideadmClassView
{
	/**
	...
	*/
	protected function displayDefault($tpl = null)
	{
		...
		$this->menu = GuideadmHelper::addSubmenu('social', 'default');
		$lists = array();
		$this->lists = &$lists;

		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_SOCIAL'));
		...
		// Label Type
		$modelLabelid = CkJModel::getInstance('sociallabels', 'GuideadmModel');
		...
		// Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_SOCIAL'), 'list');

		// New
		if ($model->canCreate())
			JToolBarHelper::addNew('socialitem.add', "GUIDEADM_JTOOLBAR_NEW");

		// Edit
		if ($model->canEdit())
			JToolBarHelper::editList('socialitem.edit', "GUIDEADM_JTOOLBAR_EDIT");

		// Delete
		if ($model->canDelete())
			JToolBarHelper::deleteList(JText::_('GUIDEADM_JTOOLBAR_ARE_YOU_SURE_TO_DELETE'), 'socialitem.delete', "GUIDEADM_JTOOLBAR_DELETE");
		...
		if ($model->canEditState())
			JToolBarHelper::publishList('social.publish', "GUIDEADM_JTOOLBAR_PUBLISH");

		// Unpublish
		if ($model->canEditState())
			JToolBarHelper::unpublishList('social.unpublish', "GUIDEADM_JTOOLBAR_UNPUBLISH");

		// Trash
		if ($model->canEditState())
			JToolBarHelper::trash('social.trash', "GUIDEADM_JTOOLBAR_TRASH");

		// Archive
		if ($model->canEditState())
			JToolBarHelper::archiveList('social.archive', "GUIDEADM_JTOOLBAR_ARCHIVE");
	}
	...
	*/
	protected function displayModal($tpl = null)
	{
		...
		$this->menu = GuideadmHelper::addSubmenu('social', 'modal');
		$lists = array();
		$this->lists = &$lists;
...
if (!class_exists('GuideadmViewSocial')){ class GuideadmViewSocial extends GuideadmCkViewSocial{} }
