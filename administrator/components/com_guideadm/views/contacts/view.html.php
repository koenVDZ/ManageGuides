...
* @subpackage	Contacts
*/
class GuideadmCkViewContacts extends GuideadmClassView
{
	/**
	...
	*/
	protected function displayDefault($tpl = null)
	{
		...
		$this->menu = GuideadmHelper::addSubmenu('contacts', 'default');
		$lists = array();
		$this->lists = &$lists;

		// Define the title
		$this->_prepareDocument(JText::_('GUIDEADM_LAYOUT_CONTACTS'));
		...
		// Country
		$modelBusiness_type_country_id = CkJModel::getInstance('countries', 'GuideadmModel');
		$modelBusiness_type_country_id->set('context', $model->get('context'));
		$filters['filter_business_type_country_id']->jdomOptions = array(
			'list' => $modelBusiness_type_country_id->getItems()
		);

		// Nationality
		$modelNationality = CkJModel::getInstance('countries', 'GuideadmModel');
		$modelNationality->set('context', $model->get('context'));
		$filters['filter_nationality']->jdomOptions = array(
			'list' => $modelNationality->getItems()
		);
		...
		// Category
		$modelVisits_id = CkJModel::getInstance('categories', 'GuideadmModel');
		$modelVisits_id->set('context', $model->get('context'));
		$filters['filter_visits_id']->jdomOptions = array(
			'list' => $modelVisits_id->getItems()
		);
		...
		// Toolbar
		JToolBarHelper::title(JText::_('GUIDEADM_LAYOUT_CONTACTS'), 'list');

		// New
		if ($model->canCreate())
			JToolBarHelper::addNew('contact.add', "GUIDEADM_JTOOLBAR_NEW");

		// Edit
		if ($model->canEdit())
			JToolBarHelper::editList('contact.edit', "GUIDEADM_JTOOLBAR_EDIT");

		// Delete
		if ($model->canDelete())
			JToolBarHelper::deleteList(JText::_('GUIDEADM_JTOOLBAR_ARE_YOU_SURE_TO_DELETE'), 'contact.delete', "GUIDEADM_JTOOLBAR_DELETE");
		...
		if ($model->canEditState())
			JToolBarHelper::publishList('contacts.publish', "GUIDEADM_JTOOLBAR_PUBLISH");

		// Unpublish
		if ($model->canEditState())
			JToolBarHelper::unpublishList('contacts.unpublish', "GUIDEADM_JTOOLBAR_UNPUBLISH");

		// Trash
		if ($model->canEditState())
			JToolBarHelper::trash('contacts.trash', "GUIDEADM_JTOOLBAR_TRASH");

		// Archive
		if ($model->canEditState())
			JToolBarHelper::archiveList('contacts.archive', "GUIDEADM_JTOOLBAR_ARCHIVE");
	}
	...
	*/
	protected function displayModal($tpl = null)
	{
		...
		$this->menu = GuideadmHelper::addSubmenu('contacts', 'modal');
		$lists = array();
		$this->lists = &$lists;
	...
	*/
	protected function getSortFields($layout = null)
	{
		return array(
			...
			'surname' => JText::_('GUIDEADM_FIELD_SURNAME'),
			'alias' => JText::_('GUIDEADM_FIELD_ALIAS'),
			'company_id.name' => JText::_('GUIDEADM_FIELD_COMPANY'),
			'business_type.country_id' => JText::_('GUIDEADM_FIELD_COUNTRY')
		);
	}
...
if (!class_exists('GuideadmViewContacts')){ class GuideadmViewContacts extends GuideadmCkViewContacts{} }
