...
* @subpackage	Classes
*/
class GuideadmCkModelAddresslabel extends GuideadmClassModelItem
{
	/**
	* View list alias
	*
	* @var string
	*/
	protected $view_item = 'addresslabel';
	...
	* @var string
	*/
	protected $view_list = 'addresslabels';
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'addresslabel', 'STRING');
	}
	...
	*/
	public function getTable($type = 'addresslabel', $prefix = 'GuideadmTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_guideadm.edit.addresslabel.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('addresslabel.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
	...
	* @param	integer	$pk	The primary id key of the addresslabel
	*
	* @return	void
	*/
	...
	{
		//FROM : Main table
		$query->from('#__guideman_addresslabels AS a');
		...
		{
			case 'layout.addresslabel':

				$this->orm->select(array(
					'created_by',
					'created_by.email',
					'created_by.lastvisitDate',
					'created_by.name',
					'created_by.registerDate',
					'created_by.username',
					'creation_date',
					'language',
					'language.title',
					...
					'modified_by',
					'modified_by.email',
					'modified_by.lastvisitDate',
					'modified_by.name',
					'modified_by.registerDate',
...
if (!class_exists('GuideadmModelAddresslabel')){ class GuideadmModelAddresslabel extends GuideadmCkModelAddresslabel{} }
