...
* @subpackage	Form
*/
class GuideadmCkFormRuleNumeric extends GuideadmClassFormRule
{
	/**
	...
	*/
	public function getJsonRule($field)
	{
		...
		else if ($numericFormat = $field->getAttribute('numericFormat'))
		{
			...
			$alertText = JText::sprintf('GUIDEADM_ERROR_INCORRECT_FORMAT_EXPECTED_DIGITS_MAX_DECIMALS_MAX', $length, $scale);
		}
		else
			// Fallback default error message for all numeric types
			$alertText = JText::sprintf('GUIDEADM_ERROR_EXPECTED_NUMERIC_FORMAT');
...
if (!class_exists('JFormRuleNumeric')){ class JFormRuleNumeric extends GuideadmCkFormRuleNumeric{} }
